...
/**
 * A jatekos tipusoknak (jelenleg mechanic es saboteur) az ososztalya. Itt
 * talalhatoak azok az
 * attributumok es fuggvenyek, amelyek minden jatekosnak vannak. Ez egy
 * absztrakt osztaly.
 */
// Tomi
public abstract class Player implements Serializable {
    ...
    /**
     * A fuggveny meghivasakor a jatekos kivalasztja a megfelelo elemet es annak
     * meghivja
    ...
    private boolean isMovementRestricted() {
        if (cantMove > 0) {
            System.out.println("A jatekos ideje, ameg nem lephet csokkent eggyel, meg " + cantMove
                    + " korig nem fog tudni elmozdulni.");
            cantMove--;
            return true;
        }
        return false;
    ...
    /**
     * Kiirja a parameterben megadott szomszedu elemrol(szomszedok listajanak a
     * p-edikrol) az
     * informaciokat(mindent amit lehet rola tudni pl.stucky-e, allnak-e mar rajta
     * stb.).
     * Pontosabban ez a fuggveny fogja meghivni az adott elemnek, amit kivalasztott
     * az Info()
    ...
    /**
     * Kiirja a jelenlegi elem informacioit(mindent amit lehet rola tudni
     * pl.stucky-e, allnak-e mar rajta
     * stb.). Pontosabban ez a fuggveny fogja meghivni az adott elemnek, amit
     * kivalasztott az Info()
     * fuggvenyet, ami fogja kiirni az adatokat az elemrol
     */
    public void Info() {
        element.Info();
